...
pub enum VerifierError<F: Field> {
    ...
    Round1SumMismatch { got: F, expected: F },

    /// Round `j` (2 ≤ j ≤ n) failed: `s_j(0) + s_j(1) ≠ s_{j-1}(r_{j-1})`.
    RoundConsistencyMismatch { round: usize, got: F, expected: F },

    /// Final oracle check failed: `s_n(r_n) ≠ oracle_eval`.
    OracleCheckFailed { got: F, expected: F },
}
...
impl Verifier {
    ...
    pub fn verify<F: Field>(
        ...
        // Degenerate zero-variable case: the Sumcheck transcript has no rounds.
        // The claimed hypercube sum is simply the oracle evaluation.
        if n == 0 {
            if proof.claimed_sum != oracle_eval {
                return Err(VerifierError::OracleCheckFailed {
                    got: proof.claimed_sum,
    ...
    pub fn verify_transcript<F: Field>(
        ...
        // A zero-variable transcript has no round-consistency checks.
        if n == 0 {
            return Ok(());
        }
...
mod tests {
    ...
    fn tampered_last_round_poly_fails_oracle() {
        ...
        proof.round_polys[2] = RoundPoly::new(correct_s3.a + fr(1), correct_s3.b);
    ...
    #[test]
    fn zero_variable_proof_verifies() {
        let proof = crate::sumcheck::proof::SumcheckProof::<Fr>::new(fr(7), vec![]);
        assert!(Verifier::verify(&proof, &[], fr(7)).is_ok());
        assert!(Verifier::verify_transcript(&proof, &[]).is_ok());
    }

    #[test]
    fn zero_variable_wrong_oracle_is_rejected() {
        let proof = crate::sumcheck::proof::SumcheckProof::<Fr>::new(fr(7), vec![]);
        assert!(matches!(
            Verifier::verify(&proof, &[], fr(8)),
            Err(VerifierError::OracleCheckFailed { .. })
        ));
    }
}
