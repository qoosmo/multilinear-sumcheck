...
use mlp_pro::poly::{CanonicalPoly, LagrangePoly};
    ...
    for n in [10, 15, 20] {
        ...
        // 1. eval_standard
        {
            ...
            group.bench_with_input(
                BenchmarkId::new("eval_standard", n),
                &n,
                |b, _| b.iter(|| poly.eval_standard(&point)),
            );
        }
        ...
        // 2. eval_optimized
        {
            let poly = LagrangePoly::new(evals.clone());
            group.bench_with_input(
                BenchmarkId::new("eval_optimized", n),
                &n,
                |b, _| b.iter(|| poly.eval_optimized(&point)),
            );
        }
        ...
        // 3. eval_parallel
        {
            let poly = LagrangePoly::new(evals.clone());
            group.bench_with_input(
                BenchmarkId::new("eval_parallel", n),
                &n,
                |b, _| b.iter(|| poly.eval_parallel(&point)),
            );
        }

        // 4. ark-poly DenseMultilinearExtension::evaluate
        {
            let ark_poly = DenseMultilinearExtension::from_evaluations_vec(
                n, evals.clone(),
            );
            group.bench_with_input(
                BenchmarkId::new("ark_poly_evaluate", n),
                &n,
                |b, _| b.iter(|| ark_poly.evaluate(&point)),
            );
        }
    }
...
fn bench_sumcheck_canonical(c: &mut Criterion) {
    ...
    for n in [10, 15, 20] {
        ...
        group.bench_with_input(
            BenchmarkId::new("prove", n),
            &n,
            |b, _| b.iter(|| prover.prove(&challenges)),
        );
        ...
        group.bench_with_input(
            BenchmarkId::new("prove_and_verify", n),
            &n,
            |b, _| b.iter(|| {
                let proof       = prover.prove(&challenges);
                let oracle_eval = f.eval_circuit(&challenges);
...
fn bench_prover_construction(c: &mut Criterion) {
    ...
    for n in [10, 15, 20] {
        ...
        group.bench_with_input(
            BenchmarkId::new("canonical_new", n),
            &n,
            |b, _| b.iter(|| CanonicalProver::new(&canon_f)),
        );

        group.bench_with_input(
            BenchmarkId::new("lagrange_new", n),
            &n,
            |b, _| b.iter(|| LagrangeProver::new(&lagrange_f)),
        );
